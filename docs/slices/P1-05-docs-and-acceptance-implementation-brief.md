...
为 P1-01 至 P1-06 提供可执行、可回溯的范围说明、手动验收步骤、开发数据库安全规则和 migration/升级/回滚草案。文档必须把已合入的 P1 代码、仍保留的 P0 Mock 和待用户手动验收证据区分开，不能把未来行为写成当前仓库事实。
...
- 当前脚本 `scripts/run.ps1` 仍启动 P0 Mock，文档必须保留这一事实；
